...
- Makes batch translation, library status, and Reader use the same visible MinerU blocks; safely restores source-bound translations written by older batches instead of showing zero translated blocks, and preserves saved translations during single-block retries.
...
- 让批量翻译、文库状态和阅读器使用同一组可见 MinerU 块；安全恢复旧批处理已写入的源绑定译文，避免打开后显示“0 个译文块”，并防止单块重译时覆盖其他已保存译文。
